...
test("user registers, logs in and checks out a Zara product", async ({ page }) => {
  ...
  await page.screenshot({ path: path.join(__dirname, "..", "..", "screenshots", "1-login.png"), fullPage: true });
  ...
  await page.screenshot({ path: path.join(__dirname, "..", "..", "screenshots", "2-add-to-cart.png"), fullPage: true });
